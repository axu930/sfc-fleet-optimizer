...
`/tools/`; currently available tools are the **Zeus Fleet Optimizer** and the
...
- `css/app.css` owns the canonical theme, shared shell, and Zeus
  optimizer/allocation-specific presentation.
...
Model modules expose browser globals and CommonJS exports. The browser loads
them in dependency order; Node tests import the same production files.

## GitHub Pages

1. In **Settings → Pages**, choose **Deploy from a branch**.
2. Select the branch (usually `main`) and `/ (root)`.
3. Open the landing page at `/`, the optimizer at `/tools/zeus-optimizer/`, and
   the allocator at `/tools/fleet-allocation/`.
